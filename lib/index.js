...
prove.prototype.test = function (val) {
    ...
    if (null == val) {
        ...
        for (var i = 0, len = this.selected.length; i < len; i += 1) {
    ...
    if (false === result) {
        return true;
    } else if (1 === Object.keys(result).length && (null != result[this.path] || null != result[''])) {
        return new ValidationError((result[this.path] || result['']).message);
    } else {
